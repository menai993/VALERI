...
def _metric_names(session: Session, role: str) -> set[str]:
    return {c.name for c in list_capabilities(session, role) if c.kind == "metric"}


def _tool_names(session: Session, role: str) -> set[str]:
    return {c.name for c in list_capabilities(session, role) if c.kind == "tool"}


def test_owner_sees_the_new_csa_metrics(db_session: Session) -> None:
    metrics = _metric_names(db_session, "owner")
...
def test_hidden_meta_tools_not_listed_as_capabilities(db_session: Session) -> None:
    tools = _tool_names(db_session, "owner")
...
def test_rep_only_sees_customer_scopable_metrics(db_session: Session) -> None:
    """A sales_rep cannot run company/segment-wide metrics (finance data, D2)."""
    rep_metrics = _metric_names(db_session, "sales_rep")
...
def test_descriptions_are_present_and_nonempty(db_session: Session) -> None:
    for cap in list_capabilities(db_session, "owner"):
