"""Narration orchestration: mask → prompt → call → validate → retry → log.

Every LLM API call (including rejected attempts) writes one audit.ai_log row.
A narration that fails schema validation or the number contract after the
configured retries raises NarrationFailed — the caller falls back to templates.
"""
...
def narrate_task(
    ...
    be validated within the retry budget. Never returns unvalidated output.
    """
