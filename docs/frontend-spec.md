...
1. Data widgets (StatCard, Sparkline, ComboChart, SubStatStrip, DataTable, AIInsightItem, EvidenceExpander, ConfidenceLabel, OwnerReportSummary, RuleCard).
1. Shell (TopBar + CompanySwitcher + GlobalSearch/Ask entry + NotificationsBell + ProfileMenu + Sidebar + QuickActions) and routing + auth guard.
1. Assemble **Početna** to match the reference; wire `useDashboard()`.
1. Remaining screens in order: Zadaci, Artikli (lost articles), Kupci, AI Report (report → learned-rules → investigations), Chat, Postavke.
1. Light/dark parity pass + accessibility pass (focus rings, keyboard nav, AA contrast, chips carry text).
