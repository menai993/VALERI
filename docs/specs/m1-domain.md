...
**Milestone:** M1 · **Builds on:** M0 (schemas exist, Alembic wired) · **Status:** awaiting review
...
1. **Seed CLI default volume** — ~80 customers / ~18 months ≈ 20–30k invoices and
   80–150k lines (a few seconds to generate, a few MB in the DB). OK, or prefer smaller?
2. **`as_of` semantics** — default `as_of = today` at seed time (planted cases always
   "fresh" relative to seeding), with `--as-of` override for reproducible CI runs. OK?
3. **Manifest committed** — `db/seed/planted_cases.json` is regenerated by the seed CLI and
   committed so M3/M4 tests have stable ground truth. OK?
