...
**Milestone:** M7 · **Builds on:** M6 (LLM narration layer exists) · **Status:** awaiting review
...
1. **D1 (`app.owner_report` table)** — approve the schema addition + data-model.md update?
2. **D2 (`app.approval.payload` column)** — approve the addition + data-model.md update?
3. **D3 (drafts for decline + sleeping tasks)** — generate win-back/offer message drafts
   during report generation (LLM, masked, approval-gated). OK?
4. **D5 (report section list)** — the 7 sections as listed. OK?
5. **Week boundary**: report week = Monday–Sunday, generated by the Sunday-night weekly scan
   job (Europe/Sarajevo). OK?
